...
export default {
    ...
    methods: {
        ...mapActions(['fetchAddresses', 'addAddress', 'deleteAddress']),
...
<template>
    ...
    <div v-else-if="toggleAddAddress ?? addresses.length != 0">
        <div v-for="address in addresses" :key="address.id" class="address-card mb-3">
            ...
            <button @click="editAddress(address)" id="btn-edit">Edit</button>
            <button @click="deleteAddress(address.id)" id="btn-delete">Delete</button>
        </div>
    </div>
